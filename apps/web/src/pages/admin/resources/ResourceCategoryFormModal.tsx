...
} from "../../../services/api/resourceCategories";
import { listAllDepartments } from "../../../services/api/departments";

/**
 * Create/edit a resource category (SYSTEM_PLAN.md §14.5/§20, Phase 5.1).
 * Mirrors CourseCategoryFormModal.tsx's exact shape — same create/update-
 * share-a-schema pattern, same field-level 409 error surfacing. `is_active`
 * is deliberately NOT a field here — activation/deactivation is a one-click
 * action on the list page.
 *
 * `department_id` (Department -> Category -> Training Content hierarchy
 * unit) mirrors CourseCategoryFormModal.tsx's identical addition — an empty
 * selection means "Global (no department)".
 */
export function ResourceCategoryFormModal({
  ...
  const {
    ...
    setError,
  } = useForm<CreateResourceCategoryRequest>({
    resolver: zodResolver(createResourceCategoryRequestSchema),
  ...
  const mutation = useMutation({
    mutationFn: async (values: CreateResourceCategoryRequest) => {
      const normalized = {
        ...values,
      ...
      return isEdit
        ? updateResourceCategory(category!.id, normalized)
        : createResourceCategory(normalized);
    },
