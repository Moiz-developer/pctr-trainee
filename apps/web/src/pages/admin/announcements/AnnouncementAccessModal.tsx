...
import type { AnnouncementAccessResponse } from "@internal-training/shared";
...
} from "../../../services/api/announcementAccess";

/**
 * Explicit per-user Announcement access (consistent granular access control
 * unit, §2/§5). Mirrors resources/ResourceAccessModal.tsx exactly — same
 * combined audit-history-table + dynamic-user-search-and-grant modal,
 * reached from a list row's "Access" button (Announcements, like Resources,
 * has no per-item admin detail page).
 */
export function AnnouncementAccessModal({
  open,
  onClose,
  announcementId,
  announcementTitle,
  ...
  const accessQuery = useQuery({
    queryKey: ["announcement-access", announcementId],
    queryFn: () => listAnnouncementAccess(announcementId),
    enabled: open,
  });
  ...
  // CourseAccessPanel.tsx/ResourceAccessModal.tsx: there is no "look up
  // users by id" endpoint, so this is built from up to 100 users, same
  // >100-user limitation).
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["announcement-access", announcementId] });
  ...
  return (
    <Modal open={open} onClose={onClose} title={`Access — ${announcementTitle}`} wide>
      <div>
      ...
      <ConfirmDialog
        open={!!revokeTarget}
        title="Revoke access"
        description="This user will lose access to this announcement. You can restore it later."
        confirmLabel="Revoke"
